...
public final class EcarxCarManager {
    ...
    public synchronized void unwatchFunction(ICarFunction.IFunctionValueWatcher watcher) {
        ...
        if (carFunction != null) {
            try {
                ...
                AppLog.d(TAG, "注销车身功能监听失败: " + t.getMessage());
            }
        }
    }
    ...
    public synchronized void unwatchSensor(ISensor.ISensorListener listener) {
        ...
        if (sensor != null) {
            try {
                ...
                AppLog.d(TAG, "注销传感监听失败: " + t.getMessage());
            }
        }
    }
    ...
    public synchronized void ensureConnected(Context context) {
        ...
        try {
            ...
            if (created == null) {
                AppLog.d(TAG, "Car.create 返回 null，车控 API 不可用");
                return;
            }
            car = created;
            if (created instanceof IConnectable) {
                ...
                connectable.connect();
                AppLog.d(TAG, "ecarx Car 连接中…");
            } else {
                // 同步可用，直接拿管理器并补注册
                onConnectedInternal();
            }
        } catch (Throwable t) {
            AppLog.d(TAG, "ecarx Car 接入失败，降级: " + t.getMessage());
            connecting = false;
        }
    }
    ...
    private synchronized void onConnectedInternal() {
        try {
            ...
            connected = true;
            connecting = false;
            AppLog.d(TAG, "ecarx Car 已连接，补注册 "
            ...
            AppLog.d(TAG, "获取车控管理器失败: " + t.getMessage());
        }
    }
    ...
    private void applyFunction(int functionId, ICarFunction.IFunctionValueWatcher watcher) {
        try {
            ...
            AppLog.d(TAG, "注册车身功能 " + functionId + " 失败: " + t.getMessage());
        }
    }
    ...
    private void applySensor(int sensorType, int zone, ISensor.ISensorListener listener) {
        try {
            ...
            AppLog.d(TAG, "注册传感 " + sensorType + " 失败: " + t.getMessage());
        }
    }
    ...
    /** 写车身功能整数值（带 zone）。未连接或异常返回 false。 */
    public synchronized boolean setFunction(int functionId, int zone, int value) {
        if (carFunction == null) {
            return false;
        }
        try {
            return carFunction.setFunctionValue(functionId, zone, value);
        } catch (Throwable t) {
            AppLog.d(TAG, "set(" + functionId + ",z=" + zone + "," + value + ") 失败: " + t.getMessage());
            return false;
        }
    }

    /** 写车身功能浮点自定义值（带 zone，如温度/车窗位置）。未连接或异常返回 false。 */
    public synchronized boolean setCustomizeFunction(int functionId, int zone, float value) {
        if (carFunction == null) {
            return false;
        }
        try {
            return carFunction.setCustomizeFunctionValue(functionId, zone, value);
        } catch (Throwable t) {
            AppLog.d(TAG, "setC(" + functionId + ",z=" + zone + "," + value + ") 失败: " + t.getMessage());
            return false;
        }
    }

    /** 读车身功能整数值（带 zone）。未连接返回 {@link Integer#MIN_VALUE}。 */
    public synchronized int readFunction(int functionId, int zone) {
        if (carFunction == null) {
            return Integer.MIN_VALUE;
        }
        try {
            return carFunction.getFunctionValue(functionId, zone);
        } catch (Throwable t) {
            return Integer.MIN_VALUE;
        }
    }

    /** 读车身功能浮点自定义值（带 zone）。未连接返回 NaN。 */
    public synchronized float readCustomizeFunction(int functionId, int zone) {
        if (carFunction == null) {
            return Float.NaN;
        }
        try {
            return carFunction.getCustomizeFunctionValue(functionId, zone);
        } catch (Throwable t) {
            return Float.NaN;
        }
    }
